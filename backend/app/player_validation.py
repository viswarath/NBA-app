...
def is_valid_player_name(name: str):
  ...
  if not name or name[0].isdigit():
    raise ValueError("Invalid name. The player's name must not start with a number.")

def is_valid_age(age: int):
  if not isinstance(age, int):
    raise ValueError(f"Invalid age: {age}. Must be an integer.")
  if age > 100:
    raise ValueError(f"Invalid age: {age}. Age must be 100 or less.")
...
def is_valid_games_started(games_started: int):
  ...
  if games_started > 82:
    raise ValueError(f"Invalid position: {games_started}. Must be less than of the number of games in a season: {NUM_GAMES_SEASON}.")
